...
begin
  ...
  //if Form4.ShowModal<>mrOk then goto m303; // если не нажат ОК, то - выходим
   ...
   s.DelimitedText := OptNodeEdt.text ; //заполняем текстом с разделителем
...
procedure TFerm_opt_massa.FormShow(Sender: TObject);
begin
Height := round(504*Main_Form.Height_Ratio);
Width  := round(614*Main_Form.Width_Ratio);
end;

procedure TFerm_opt_massa.FormActivate(Sender: TObject);
begin
...
procedure TFerm_opt_massa.FormDeactivate(Sender: TObject);
begin
  if check then
    Main_Form.NodesNum.Checked := True
  else
...
procedure TFerm_opt_massa.FormDestroy(Sender: TObject);
begin
  if check then
    Main_Form.NodesNum.Checked := True
  else
