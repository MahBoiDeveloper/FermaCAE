...
type
 ...
 const AFromDirectory, AToDirectory: string);
  ...
  public
    ...
    { Public declarations }
    Plast:TPlast;
    FileName,rez_name:string; // Имя файла с данными
    CurrentNode:integer;     // Узел, на котором щелкнули последний раз
    isChanged:boolean;
    isNamed:boolean;
    SaveCancel:boolean;       // Отменили ли мы сохранение ?
    CW,CH: integer;
    Other_MDopNapr,Other_MModUpr,Other_MKoefPuass:extended;    // Переменная для хранения неизвестного(временного) материала
    Xold,yold:integer;  //старые координаты предварительных линий
    razbit:boolean; //=true, если пластину только что разбивали
    zagr:boolean;   //=true, если курсор за границей пластины с рамкой
    nz: integer;    // номер случая нагружения или закрепления
    ng :integer;    // номер случая нагружения
    ...
    old1,old2:integer; //старые значения силл для удаленной точки
    {Удаление разбиения}
     cutX1_x, cutX2_x,
     ...
     cutIdent:string;
    procedure deleteVar(ngr:integer);     //Процедура удаления случая нагружения со всеми его точками
    procedure LoadData;                   // Загрузка данных их файла в структуры проэкта
    procedure SaveData;                   //Сохранение данных в файле
    procedure initialize;                 //Инициализация Пластины данными из Nagruz
...
begin
  ...
  isNamed:=false;
  kl:=0;
  for i:= 1 to 3 do Kt[i]:=0;            // Обнуление всей информации о нагружениях
  for i:= 1 to 3 do                      //
  for j:=1 to 3 do                       //
  for k:=1 to 4 do                       //
  nagruz[i,j,k]:=0;                      //
...
begin
  ...
  Main_Form.Sn_Cbx.Items.Add('Случай нагружения 1');
...
var i,j,k,t:integer;
 ...
 repaint;
  ...
  procedure TPlast_Form.deleteNode(node,ng:integer); //Удаление точки Node из случая нагружения Ngr
var i,j,k,t:integer;
begin
...
var
  ...
  {readln(ff);
  readln(ff);  }
  readln(ff,plast.e1[1]); // мод. упругости
  readln(ff,plast.e1[2]); // коэфф. Пуассона
  readln(ff,plast.dop1);  // Доп. напр.
  readln(ff,plast.ton1);  // нач. толщина
  readln(ff,plast.kz1);   // число закреплений
//readln(ff,plast.kl1); // нагружения
  ...
  readln(ff,plast.kx1);       // число разбиений по Х
99:  readln(ff,plast.ky1);   // число разбиений по Y
...
{ for j:=1 to plast.kl1 do           //Теперь читаем данные не на прямую в plast,а сначала
begin                                в массив nagruz,а заетем уже инициализируем Plast данными
      read(ff,plast.nomm[j]);        из nagruz
      read(ff,plast.os[j]);
...
//Создаем одну большую пластину с 9-ю точками нагр.
initialize;
...
i: integer; }
begin
     visio_form.num_color:=5; //Количество уровней цветов
     visio_form.num_force:=1; //Случай нагружения
     visio_form.num_value:=0; //Смотрим мах. экв. напряжение
     visio_form.h_or_w_color:=0; //Вертикальная шкала цветов
      Main_Form.Sn_Cbx.clear;
//      visio_form.Dote_Rez_Btn.down:=true;
         razbit:=false; // пластину только что не разбивали !
      zagr:=true; //курсор за границей пластины и рамки
     Main_Form.Sn_Cbx.clear; Main_Form.sn_cbx.Items.Add('Случай нагружения 1');
     Main_Form.Sn_Cbx.ItemIndex:=0;
...
begin
 ...
 if isChanged then
  begin
   Flag:=MessageDlg(chr(13)+'Сохранить изменения в файле "' + Caption + '"?     ' ,mtWarning,mbYesNoCancel,0);
...
// Процедура рисования закреплений для пластины
...
begin
 ...
 end; // закрываем with do
end; // закрываем процедуру


//Процедура рисования нагружений для пластины
procedure TPlast_Form.force_draw_for_plast(x_max,y_max:integer;mas_x,mas_y:single); //Рисуем нагружения
var
...
//  for i:=1 to ii do // Изменено в ферме 4- так как при старом варианте рисовалось от 1 до выбранного случая нагружения все варианты а надо то только выбранный.
// а теперь мы рисуем только те точки пластины,которые относятся к текущему случаю нагружения
...
var
  ...
  step,
  mas_x,mas_y:single;     //Масштабирующие коэффициенты
  number,sss:string;
  n_of_step:integer;
begin
  ...
  begin
    if Main_Form.NodesNum.Checked or Main_Form.Knum.Checked then
     begin
      ...
      font.Color:=clGreen;
      font.size:=7;
      if plast.ky1>plast.kx1 then //нумеруем по горизонтали
      begin
      for i:=1 to plast.ky1 do
       begin
...
begin
 ...
 { Main_Form.Plast_ToolBar.Left:=86;
  ...
  //Main_Form.Caption:='Ферма - подсистема '+#39+'Пластина'+#39;
...
Version_VYV:string;
 ...
 mg:integer;
begin
Visio_Form.plast:=plast; // Самойленко А.Е. 11.11.07
// Проверка на существование файлов расчета
  ...
  if Version_VYV='Error' then
   begin
    Beep;
    MessageDlg(#13+'Файлы с оптимизационным расчетом не найдены.',mtError,[mbOk],0);
    exit;
   end;

// Проверка на совместимость версий файлов данных и оптимизационного рассчета
if FileExists(FileName) then
 begin
  Assignfile(ff,FileName);
  Reset(ff);
 ...
 begin
    Beep;
    MessageDlg('Несовпадение версий файлов с данными и оптимизационного расчета.'+#10+#13+'Произведите оптимизационный расчет для данной пластины.',mtError,[mbOk],0);
    exit;
 end;
 ...
 if ((Version_VYV)<>(ExtractFileName(FileName)+' '+IntToStr(mg)+' '+DateTimeToStr(FileDateToDateTime(FileAge(FileName)))))
  ...
  Visio_Form.num_value:=6;//толщина Visio_Form.num_rast:=1; //оптимизация
  Visio_Form.num_rast:=1; //оптимизация
  ...
  for i:=2 to plast.kx1-1 do
    ...
    for j:=2 to plast.ky1-1 do
      begin
       ...
       if (currentnx=0)and(currentny=0)then exit;// выход, если не удаляем
        if currentnx<>0 then
        begin
        ...
        //  for j=1 to plast.kx1 do
          if   (Plast.kx1>=plast.ky1) and (plast.zak1[i]>=(currentnx-1)*plast.ky1+1) and (plast.zak1[i]<= (currentnx)*plast.ky1)then
            begin
              Main_Form.StatusBar1.Panels[1].Text := 'Невозможно. Присутствует закрепление.';
              Main_Form.StatusBar1.Panels[2].Text := 'Переместите курсор.';
              Exit;
            end;
            for j:=1 to plast.ky1 do
         ...
         begin
          if   (Plast.kx1>=plast.ky1) and (plast.nomm[i]>=(currentnx-1)*plast.ky1+1) and (plast.Nomm[i]<= (currentnx)*plast.ky1)then
            begin
              Main_Form.StatusBar1.Panels[1].Text := 'Невозможно. Присутствует сила.';
              Main_Form.StatusBar1.Panels[2].Text := 'Переместите курсор.';
              Exit;
            end;
            for j:=1 to plast.ky1 do
...
begin
 ...
 if ((RealRealX<Plast.xm1[plast.kx1])and(RealRealX>0)) or ((RealRealY<Plast.ym1[plast.ky1])and(RealRealY>0)) then //если курсор в нужной области
  begin
   ...
   for i:=1 to plast.kx1 do
    if (realX=round(plast.xm1[i])) then
     begin
      Main_Form.StatusBar1.Panels[1].Text := 'Невозможно разбить пластину в этом месте.';
      Main_Form.StatusBar1.Panels[2].Text := 'Переместите курсор.';
      Exit;
...
end // Конец "если курсор в нужной области"
...
//Формируем новые разбиения
 ...
 begin
  ...
  begin  //проверка на мах разбиений
    Main_Form.StatusBar1.Panels[0].Text := '';
    Main_Form.StatusBar1.Panels[1].Text := 'Достигнуто максимальное число узлов.';
...
var
  i,nz:integer;
  z1,z2,z3:array[1..plast_max_zak] of integer; // Закрепления (номер узла, X и Y)
begin
  if CurrentNode=0 then Exit;
  nz:=0;
  for i:=1 to plast.kz1 do
  ...
  if nz=0 then
    begin
      if plast.kz1=plast_max_zak then
        begin
          MessageDlg(#13+'Слишком много закрепленных узлов.',mtWarning,[mbOk],0);
          Exit;
        end;
      inc(plast.kz1);
      nz:=plast.kz1;
...
///////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////
///////////////// Нагружения через панель построения //////////////////
///////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////
...
var
  ...
  begin
  if CurrentNode=0 then Exit;
//Теперь все делается совсем по другому
//nz:=0;
//  for i:=1 to plast.kl1 do
...
//        begin
//          MessageDlg(#13+'Слишком много нагруженных узлов.',mtWarning,[mbOk],0);
//          Exit;
//        end;
    if kl=0 then
      begin
...
var
  ...
  i,j,k,ii:integer;
begin
//сохраняем основной файл проекта
 begin
  AssignFile(ff,FileName);
  Rewrite(ff);
...
//writeln(ff,plast.kl1);
  writeln(ff,kl);   // число случаев нагружения
  for i:=1 to kl do
  begin
  writeln(ff,kt[i]);           // Кол-во точек в 1-ом случае,Кол-во точек во 2-ом случае,Кол-во точек в 3-ем случае
  end;
...
//Plast - используется только для отображения сил,все данные о силах лежат в массиве nagruz
...
begin
  ...
  if (plast.kl1 = 0) or (plast.kz1 = 0) then
   begin
    Beep;
    MessageDlg('Отсутствуют закрепления и/или силы.'+#10+#13+'Расчет невозможен.',mtError,[mbOk],0);
    Exit;
   end;
  ...
  if LibHandle=0 then
    begin
    beep;
      MessageDlg('Нет библиотеки Plastina_r.dll',mtError,[mbOk],0);
      Exit;
    end;
 ...
 { finally
    ...
    messagedlg('Ошибка в расчетном модуле.'+#10+#13+'Для продолжения работы перезапустите систему.',mtError,[mbok],0);
  end;
   Cursor:=crDefault;
     erase(dopd);
  main_form.P2.Enabled:=true;
  main_form.P3.Enabled:=true;
//end of my insert
  //MessageDlg(chr(13)+'Расчет закончен.',mtInformation,[mbOk],0);
...
8:   // Информация об узле (о конечном элементе)
begin
    Main_Form.StatusBar1.Panels[0].Text :='';
    Main_Form.StatusBar1.Panels[1].Text :='';
    if CurrentNode=0 then application.CancelHint;
    if CurrentNode<>0 then  {моя проверка}
      Main_Form.StatusBar1.Panels[1].Text :='Узел №'+IntToStr(CurrentNode)+Format(' [%g : %g]',[uxx, uyy]) // Самойленко А.Е. 11.11.07
      {конец моей проверки}// Plast_M.TPlast_Form(Main_form.ActiveMDIChild).paintbox.Hint:='Узел № '+inttostr(CurrentNode);
    else      // Самойленко А.Е. 11.11.07
     begin   // Самойленко А.Е. 11.11.07
      x_max:=PaintBox.width;
      y_max:=PaintBox.height;
      mas_x:=(x_max-80-coord_axis_x)/max_x_coord; //Масштабирующие коэффициенты
      mas_y:=(y_max-60-coord_axis_y)/max_y_coord;
      {mas_x:=(x_max-80)/max_x_coord; //Масштабирующие коэффициенты
       ...
       if (X>=50+round(mas_x*plast.xm1[i])+coord_axis_x)and(X<=50+round(mas_x*plast.xm1[i+1])+coord_axis_x)and(Y<=y_max-30-round(mas_y*plast.ym1[j])-coord_axis_y)and(Y>=y_max-30-round(mas_y*plast.ym1[j+1])-coord_axis_y)then
        begin
         if plast.kx1>=plast.ky1 then nn:=j+(i-1)*(plast.ky1-1)
          ...
          Main_Form.StatusBar1.Panels[1].Text := 'Конечный элемент №'+inttostr(nn);
          break;
       end;
     end; // Самойленко А.Е. 11.11.07
 end;
 ...
 //Main_Form.StatusBar1.Panels[1].Text := '';   закоментировано Самойленко А.Е. 11.11.07. Третья строчка от
 // константы выбора '8:' уже обеспечивает тоже самое.
end;



2:   //Рисуем предварительные линии разбивки
 begin
 ...
 // Реальные координаты второго узла
    ...
    RealX0:=(Xold-50-coord_axis_x)*max_x_coord/(PaintBox.Width-80-coord_axis_x);           //старые
    ...
    //!!! стираем линию по выходе за границу пластины !!!!!!!!!
...
RealX:=Round((X-50-coord_axis_x)*max_x_coord/(PaintBox.Width-80-coord_axis_x));
    RealY:=Round((PaintBox.Height-Y-30-coord_axis_y)*max_y_coord/(PaintBox.Height-60-coord_axis_y));
    //RealX0:=Round((Xold-50-coord_axis_x)*max_x_coord/(PaintBox.Width-80-coord_axis_x));           //старые
...
//========== Хватаем разбивку ==============
    ...
    y2_x:=-1;
     for i:=2 to plast.kx1-1 do
     begin
        //А правильно ли вычисляется нижеследующее ???
        ux:=50+coord_axis_x+(width-91-coord_axis_x)/max_x_coord*(plast.xm1[i]);
        if (X>ux-4)and(X<ux+4) then
          begin
            //Cursor:=crhandpoint;
            ...
            cutOk_:=1;
            cut_msg:='Удалить узлы по X ['+FloatToStr(plast.xm1[i])+' : -]';
      ...
      begin
        ...
        if (Y>uy-4)and(Y<uy+4) then
          begin
            ...
            cutOk_:=1;
            if cut_msg='' then
             cut_msg:='Удалить узлы по Y [- :'+FloatToStr(plast.ym1[j])+']'
            else
             cut_msg:='Удалить узлы по XY ['+FloatToStr(plast.xm1[i])+' : '+FloatToStr(plast.ym1[j])+']';
...
// Проверка на существование файлов расчета
  ...
  if Version_VYV='Error' then
   begin
    Beep;
    MessageDlg(#13+'Файлы с простым расчетом не найдены.',mtError,[mbOk],0);
    exit;
   end;
...
if FileExists(FileName) then
 ...
 begin
    Beep;
    MessageDlg('Несовпадение версий файлов с данными и простого расчета.'+#10+#13+'Произведите простой расчет для данной пластины.',mtError,[mbOk],0);
    exit;
 end;
...
if FileExists(Filename) then
 ...
 if ((Version_VYV)<>(ExtractFilename(Filename)+' '+IntToStr(mg)+' '+DateTimeToStr(FileDateToDateTime(FileAge(Filename)))))
  ...
  Visio_Form.num_value:=4;// эквивалентное напряжение
  Visio_Form.num_rast:=0; //простой расчет
  ...
  Visio_Form.LoadData(Filename);
       ...
       Visio_Form.Panel1.Visible:=True;
       Visio_Form.fnn:=Filename;
       Visio_Form.to_read_or_not_to_read:=true;// читаем исходные файлы
       Visio_Form.ShowModal
end;
end;
...
var
        ...
        s1,s:string;
begin
//Раньше силовой вес ТОК читался из .oup теперь читается из .vrp
     ...
     repeat
                readln(fff,s1);
if (s1 = 'Cил. вес теор. опт. констр.:') then readln(fff,s);

     until(eof(fff)=true);
//                readln(fff);
//                readln(fff,s);
                messageDlg('Силовой вес конструкции равен'+#10+#13+#10+#13+'P ='+s+' [Н*см]',mtInformation,[mbOk],0);


                CloseFile(fff);
        end
        else    messageDlg('Требуется произвести простой расчет пластины',mtError,[mbOk],0);
