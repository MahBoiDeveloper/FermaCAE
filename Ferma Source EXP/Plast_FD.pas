...
var
  ...
  {Переменные активного окна Пластины}
      p: TPlast;
  ...
  {Глобальные переменные для области}
      s_num:extended;
       ...
       ACol1,ARow1:integer; // колонка и столбец удаляемого разбиения
{моё добавленьице}
...
//exit;
   ...
   if (plast.kx1>=12) and (plast.ky1>=12) then
    ...
    X_Label.Caption:='Размер по X  ['+Plast.s_lin+']';
    Y_Label.Caption:='Размер по Y  ['+Plast.s_lin+']';
    if Plast<>nil then
    begin
        kx:=Plast.kx1;
        ky:=Plast.ky1;
    ...
    ton1_l.Caption:='Нач. толщина ['+Plast.s_lin+']';
    max_x_coord:=0;
    max_y_coord:=0;
    ...
    for i:=1 to 2+Main_Form.plast_num_mat do
      ...
      {Material_ComboBox.Items[i-1]:=Main_Form.plast_mmaterials[i].MName; мой коммент}
     ...
     c:=Main_Form.plast_mmaterials[i].MKoffPuas;
      if (FloatToStr(Plast.e1[1])=FloatToStr(a)) and (FloatToStr(Plast.dop1)=FloatToStr(b)) and (FloatToStr(Plast.e1[2])=FloatToStr(c)) then
       ...
       if not mat_live then
        begin
           Material_ComboBox.Items[3+Main_Form.plast_num_mat-1]:='<другой>';
    ...
    ModUpr_Label.Caption:='Модуль упругости ['+Plast.s_for+'/'+Plast.s_lin+'**2]';
    DopNapr_Label.Caption:='Допускаемое напряжение ['+Plast.s_for+'/'+Plast.s_lin+'**2]';
    ModUpr_Edit.Text:=FloatToStr(Plast.e1[1]);
    KoefPuass_Edt.Text:=FloatToStr(Plast.e1[2]);
    DopNapr_Edit.Text:=FloatToStr(Plast.dop1);

    // Топология (Разбиения по х)
    Edit_NCut_by_x.text:=IntToStr(Plast.kx1);

    Cut_by_x_coords.Cells[0,0]:='№ Разб.';
    Cut_by_x_coords.Cells[1,0]:='Коорд.разбиений по X, '+Plast.s_lin;
   ...
   for i:=1 to plast.kz1 do
    begin
     if (plast.zak2[i]=1) and (plast.zak3[i]=1) then Zakr_Grd_FD.Cells[1,i]:='Закреплен по X и Y';
     if (plast.zak2[i]=1) and (plast.zak3[i]=0) then Zakr_Grd_FD.Cells[1,i]:='Закреплен по X';
     if (plast.zak2[i]=0) and (plast.zak3[i]=1) then Zakr_Grd_FD.Cells[1,i]:='Закреплен по Y';
    end;
  ...
  //??????????????????????????????????????????
   ...
   Nagr_Grd_FD.Cells[1,0]:='Сила по X ['+Plast.s_for+']';
   Nagr_Grd_FD.Cells[2,0]:='Сила по Y ['+Plast.s_for+']';
...
//     Nagr_Grd_FD.Options:=[goFixedVertLine,goFixedHorzLine,goVertLine,goHorzLine]
   ...
   // Топология (Разбиения по х)
    ...
    Cut_by_y_coords.Cells[0,0]:='№ Разб.';
    Cut_by_y_coords.Cells[1,0]:='Коорд.разбиений по Y, '+plast.s_lin;
...
begin
 ...
 if Zakr_Grd_FD.Cells[Zakr_Grd_ACol,Zakr_Grd_ARow]='Не закреплен' then
 begin
 p.kz1:= p.kz1-1;
 if Zakr_Grd_FD.Cells[1,Zakr_Grd_ARow+1]='Закреплен по X' then Zakr_Combobox_FD.ItemIndex:=1;
 if Zakr_Grd_FD.Cells[1,Zakr_Grd_ARow+1]='Закреплен по Y' then Zakr_Combobox_FD.ItemIndex:=2;
 if Zakr_Grd_FD.Cells[1,Zakr_Grd_ARow+1]='Закреплен по X и Y' then Zakr_Combobox_FD.ItemIndex:=3;
    for i:=1 to p.kz1 do
 ...
 zzz:
 if Zakr_Grd_FD.Cells[Zakr_Grd_ACol,Zakr_Grd_ARow]='Закреплен по X' then begin p.zak2[Zakr_Grd_ARow]:=1;p.zak3[Zakr_Grd_ARow]:=0; end;
 if Zakr_Grd_FD.Cells[Zakr_Grd_ACol,Zakr_Grd_ARow]='Закреплен по Y' then begin p.zak2[Zakr_Grd_ARow]:=0;p.zak3[Zakr_Grd_ARow]:=1; end;
 if Zakr_Grd_FD.Cells[Zakr_Grd_ACol,Zakr_Grd_ARow]='Закреплен по X и Y' then begin p.zak2[Zakr_Grd_ARow]:=1;p.zak3[Zakr_Grd_ARow]:=1; end;
...
begin
 New_Cut_Form.OS_Label.caption:='Координата по X';
 New_Cut_Form.caption:='Новое разбиение по X';
 New_Cut_Form.Showmodal;
end;

procedure TPlast_FD_Form.Add_yClick(Sender: TObject);
begin
 New_Cut_Form.OS_Label.caption:='Координата по Y';
 New_Cut_Form.caption:='Новое разбиение по Y';
 New_Cut_Form.Showmodal;
end;
...
begin
        ...
        //проверка закреплений
         ...
         begin
          if   (p.kx1>=p.ky1) and (p.zak1[i]>=(currentnx-1)*p.ky1+1) and (p.zak1[i]<= (currentnx)*p.ky1)then
            begin
              Main_Form.StatusBar1.Panels[1].Text := 'На линии разбиения присутствует закрепление.';
              Main_Form.StatusBar1.Panels[2].Text := 'Выберите другую линию.';
              beep;
              Exit;
            end;
            for j:=1 to p.ky1 do
         ...
         begin
          if   (p.kx1>=p.ky1) and (p.nomm[i]>=(currentnx-1)*p.ky1+1) and (p.Nomm[i]<= (currentnx)*p.ky1)then
            begin
              Main_Form.StatusBar1.Panels[1].Text := 'На линии разбиения присутствует сила.';
              Main_Form.StatusBar1.Panels[2].Text := 'Выберите другую линию.';
              beep;
              Exit;
            end;
   ...
   iii:=1;
   jjj:=1;
   if p.kx1>=p.ky1 then  //если нумерация по вертикали
    begin
  ...
  begin
   Cut_by_y_coords.MouseToCell(X,Y,ACol1,ARow1);
    ...
    if (ARow1=1) or (ARow1=p.ky1) then
     begin
      //Main_Form.StatusBar1.Panels[1].Text := 'Невозможно удалить граничное разбиение.';
      //Main_Form.StatusBar1.Panels[2].Text := 'Выберите другое разбиение.';
      //beep;
      //exit;
       dely.Enabled:=false;
     end;
...
begin
 ...
 addforce_form.Caption:='Добавить нагружение';
...
begin
 ...
 addforce_form.Caption:='Изменить нагружение';
 addforce_form.Label3.Caption:='<= '+inttostr(p.kx1*p.ky1);
 addforce_form.ShowModal;
end;
...
begin
ShowMessage('Измените тип закрепления на "Не закреплен"');
end;
...
begin
  if p.kl1=3 then
    begin
      MessageDlg('Слишком много случаев нагружения',mtWarning,[mbOk],0);
      Exit;
    end;
  addforce_form.Caption:='Добавить случай нагружения';
  addforce_form.Label3.Caption:='<= '+inttostr(p.kx1*p.ky1);
  addforce_form.number.text:='';
  inc(Plast_M.TPlast_Form(Main_Form.ActiveMDIChild).kl);
  Main_Form.Sn_Cbx.Items.clear;
      for i:=1 to Plast_M.TPlast_Form(Main_Form.ActiveMDIChild).kl do  Main_Form.Sn_Cbx.Items.Add('Случай нагружения '+IntToStr(i));
...
begin
form:=Plast_M.TPlast_Form(Main_Form.ActiveMDIChild);
//ShowMessage('Для удаления нажмите на кнопку "Изменить"'+#10#13+'затем выберете № узла и в появившемся окне обнулите значения нагрузок');
...
begin
ShowMessage('Нажмите правую кнопку мыши, на выбранном разбиении '+#10#13+'и выберете пункт "Удалить".'+#10#13+' Удаление возможно, только если к данному разбиению не привязаны нагрузки и закрепления!');
end;
...
begin
 New_Cut_Form.OS_Label.caption:='Количество узлов по X';
 New_Cut_Form.caption:='Новое разбиение по X';
...
begin
 New_Cut_Form.OS_Label.caption:='Количество узлов по Y';
 New_Cut_Form.caption:='Новое разбиение по Y';
 New_Cut_Form.Label3.Caption  :='<=12';
 New_Cut_Form.Showmodal;
end;
