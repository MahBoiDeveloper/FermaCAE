...
time:real;
sss:string;
kl,km,kn,kb,kx,ky,kz: integer;                            {‚­Ґи­ЁҐ}
      kit:                array[1..3] of integer;      {‚­Ґи­ЁҐ}
      uq,pq:              array[1..400,1..3] of real;  {‚­Ґи­ЁҐ}
      e:                  array[1..2] of real;          {‚­Ґи­ЁҐ}
...
const rr1='.vop';
 var{ dop,ep: real; }
    w,vvv,f,fff,ffff: system.text;                                {‚­Ґи­ЁҐ}
    {  kl,km,kn,kb: integer;                            {‚­Ґи­ЁҐ}
    {  kit:                array[1..3] of integer;      {‚­Ґи­ЁҐ}
    {  uq,pq:              array[1..400,1..3] of real;  {‚­Ґи­ЁҐ}
    {  e:                  array[1..2] of real;          {‚­Ґи­ЁҐ}
    {  tm:                 array[1..200] of real;   }
        ...
        k[3,3]:=(1-e[2])/2;
          for i:=1 to 3 do
            for j:=1 to 3 do
              k[i,j]:=s1*k[i,j];
        ns:=1;
         if kit[3]=1 then //если простой расчет
         begin
         ...
         assignfile(fff,name1+'.w3');rewrite(fff);
          ...
          for n:=1 to kn do       {Начало цикла 1}
            begin
              j:=kit[1];
                for i:=1 to 50 do       {Начало цикла 2}
                  if ki<>j then j:=j+kit[2] {Понять эту строчку}
                  else goto 100;        {Конец цикла 2}
              goto 101;
    100:        if n=1 then goto 101; {На самом деле переделать 101 на 102}
              s2:=n;
              ve:=kl;
              s1:=1+(s2*(ve+2))/(49+ve);
              yyy:=int(s1);
              ...
              xx:=yyy;
                ...
                if xx<>s1 then goto 101;   {Переделать CEIL}
              ns:=s1;
 {   102:      goto 73;}
    101:        for i:=1 to 4 do        {Начало цикла  3}
                  begin
                    j:=top[n,i];
                    x[i]:=xy[j,1];
                    y[i]:=xy[j,2];
               ...
               ss:=0;
                ...
                for i:=1 to 4 do        {Начало цикла 4}
                  begin
                    j:=top[n,i];
                      for l:=1 to kl do {Начало цикла 5}
                        begin
    ...
    103:      a[1,1]:=1;
              ...
              a[3,3]:=1;
              a[4,4]:=1;
                for i:=1 to 4{7} do        {Начало цикла 6}
                  begin
                    a[5,2*(i-1)+1]:=0.25;
                    if i>=4{7} then goto 1;
                 {   i:=i+1;  }
                  end;                  {Конец цикла 6}
1:              for j:=2 to 5{8} do         {Ќ з «® жЁЄ«  7}
                  begin
                    a[6,2*(j-1)]:=0.25;
...
2:  {***************************************************************}
  {************************Џа®жҐ¤га  STRIM************************}
  {***************************************************************}
    104:        for i:=1 to 4 do          {Ќ з «® жЁЄ«  8}
                  for j:=1 to 8 do        {Ќ з «® жЁЄ«  9}
                    a[i,j]:=0;
                                          {Љ®­Ґж жЁЄ«®ў 8 Ё 9}
              a[1,3]:=1;
              a[2,4]:=1;
   ...
   { 999:      goto31;}
     112:
            ...
            if kit[3] > 1 then
            begin
            assignfile(w,name1+'.w'); //если оптимизация
             if ki = kit[3] then
              begin
            ...
            append(ffff);
            assignfile(w,name1+'.ww'); //если простой расчет
            end;
            append(w);
            writeln(vvv,' ');
            //!!!!!!!!!!!!!!!!!!!!
            writeln(vvv,'tm[',n,']=');
            writeln(vvv,tm[n]);writeln(vvv,'  - Толщина');
            if ki=kit[3] then
               if ki<>1 then
                  writeln(w,tm[n])
               else writeln(w,sem);
            ...
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                 ...
                 //!!!!!!!!!!!!!!!!!!!!!!!!
            writeln(vvv,sem);writeln(vvv,'  - Макс. экв. напряж.');
            writeln(vvv,ve);writeln(vvv,'  - Объем');
            writeln(vvv,gf);writeln(vvv,'  - Сил. вес');
      212:  tm[n]:=tm[n]*sem/dop;
        ...
        writeln(vvv);
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
        writeln(vvv,'Номер итерации:');
        writeln(vvv,ki);
        writeln(vvv,'Максимальное экв. напряжение: ');
        writeln(vvv,semm);
        writeln(vvv,'Потребный по прочности объем: ');
        writeln(vvv,s1);
        writeln(vvv,'Объем теор. опт. конструкции:');
        writeln(vvv,ves);
        writeln(vvv,'Cил. вес теор. опт. констр.:');
        writeln(vvv,ges);
        writeln(vvv,'Погрешность оптимизации (%): ');
        writeln(vvv,s2);
        writeln(vvv);
        closefile(vvv);
        // if kit[3]=1 then   //если простой расчет!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if kit[3]=ki then   //если любой расчет (последняя итерация)
         begin
            closefile(fff);
            closefile(ff);
            closefile(f);
...
const rr1='.vop';
  var
    ...
    { kn,kb,km,kl: integer;                      {‚­Ґи­ЁҐ}
    { ep: real;                         {‚­Ґи­ЁҐ}
      gc: array[1..8190] of real;
      k: array[1..3,1..3] of real;
    { tm: array[1..200] of single;         {‚­Ґи­ЁҐ}
    { top: array[1..200,1..4] of integer;  {‚­Ґи­ЁҐ}
    { xy: array[1..250,1..2] of real;   {‚­Ґи­ЁҐ}
    { e: array[1..2] of real;           {‚­Ґи­ЁҐ}
    { uq,pq: array[1..400,1..3] of real;  ‚­Ґи­ЁҐ  400,3}
    { uk: array[1..400] of real;  {‚­Ґи­ЁҐ}
      x,y: array[1..4] of real;
...
11:         for l:=1 to kl do
            begin
              writeln(vvv,'Перемещения, случай нагружения ',l);
              //!!!!!!!!!!!!!!!!!!!!!!!!!!!
                for j:=1 to km do
                begin
                  writeln(vvv,j,'u');
...
10:    writeln(vvv,'Программа прошла');
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       writeln(vvv,'Диагональный коэ-т: ');
       writeln(vvv,gc[kb*i]);
       closefile(vvv);
       {readln(ccc);}
12: end; {mage3}
...
const rr2='.vop';
   ...
   var
    ...
    reset(fff);
    readln(fff,kit[3]); {Љ®«-ў® ЁвҐа жЁ©}
    readln(fff,ep); {¤®ЇгбвЁ¬ п в®«йЁ­ }
    closefile(fff);
      if kit[3]>1 then
      begin
      assignfile(w,name1+'.w');  //оптимизация
      assignfile(ffff,name1+'.w0');
      rewrite(ffff);
      end
         else
         begin
         assignfile(w,name1+'.ww'); //простой расчет
      ...
      rewrite(w);
        ...
        {   readln(f);
          readln(f);}
          readln(f,e[1]);   {Њ®¤г«м гЇагЈ®бвЁ}
          readln(f,e[2]);   {Љ®нддЁжЁҐ­в Їг бб®­ }
          readln(f,dop);{„®Ї®«­ЁвҐ«м­®Ґ ­ Їап¦Ґ­ЁҐ}
          readln(f,ton); {Ќ з «м­ п в®«йЁ­ }

          readln(f,kz); {—Ёб«® б«гз Ґў § ЄаҐЇ«Ґ­Ё©}
//          readln(f,kl); {—Ёб«® б«гз Ґў ­ Јаг¦Ґ­Ёп}
  readln(f,kl);   // число случаев нагружения
  for i:=1 to kl do
  begin
  readln(f,kt[i]);  // Кол-во точек в 1-ом случае,Кол-во точек во 2-ом случае,Кол-во точек в 3-ем случае
  end;

          readln(f,kx); {—Ёб«® г§«®ў ­  е}
          writeln(w,kx-1);
          readln(f,ky); {—Ёб«® г§«®ў ­  y}
          writeln(w,ky-1);
          closefile(w);
          //if kit[3]=1 then
           begin
        ...
        WRITELN(www,ExtractFileName(name2),' ',mg,' ',DateTimeToStr(FileDateToDateTime(FileAge(name2))));
          ...
          writeln(www,kx);
          writeln(www,ky);
          writeln(www,kz);//?еще раз kit[3]?? теперь KZ !!!!!!!!!!!!!!!
          writeln(www,kl);
          for i:=1 to kx do
    begin
          readln(f,xm[i]);    {Є®®а¤Ё­ вл г§«®ў Ї® е}
          writeln(www,xm[i]);
       {   write(www,' ');}
       {   writeln;         }
    end;
          for i:=1 to ky do
    begin
          readln(f,ym[i]);    {Є®®а¤Ё­ вл г§«®ў Ї® y}
          writeln(www,ym[i]);
    end;
          kn:=(ky-1)*(kx-1);
          km:=kx*ky;
