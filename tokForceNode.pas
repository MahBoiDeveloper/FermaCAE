...
begin
  ...
  LabelNsn.Caption:='из '+IntToStr(tok_M.Ttok_Form(Main_Form.ActiveMDIChild).tok.nsm);

  node:=tok_M.Ttok_Form(Main_Form.ActiveMDIChild).CurrentNode;
  Node_Label.Caption:='Узел №'+IntToStr(node)+' ['+inttostr(tok_M.Ttok_Form(Main_Form.ActiveMdiChild).Xtok)+','+inttostr(tok_M.Ttok_Form(Main_Form.ActiveMdiChild).Ytok)+']';
